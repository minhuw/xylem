...
pub fn create_protocol(name: &str, http_config: Option<(&str, &str)>) -> Result<MultiProtocol> {
    match name {
        "redis" => Ok(MultiProtocol::Redis(xylem_protocols::redis::RedisProtocol::new(
...
mod tests {
    ...
    fn test_create_redis_protocol() {
        let protocol = create_protocol("redis", None);
        assert!(protocol.is_ok());
        assert_eq!(protocol.unwrap().name(), "redis");
    }

    #[test]
    fn test_create_memcached_binary_protocol() {
        let protocol = create_protocol("memcached-binary", None);
        assert!(protocol.is_ok());
    ...
    fn test_create_memcached_ascii_protocol() {
        let protocol = create_protocol("memcached-ascii", None);
        assert!(protocol.is_ok());
    ...
    fn test_create_http_protocol() {
        let protocol = create_protocol("http", Some(("/", "localhost:8080")));
        assert!(protocol.is_ok());
    ...
    fn test_create_http_protocol_without_config_fails() {
        let protocol = create_protocol("http", None);
        assert!(protocol.is_err());
    }

    #[test]
    fn test_create_xylem_echo_protocol() {
        let protocol = create_protocol("xylem-echo", None);
        assert!(protocol.is_ok());
    ...
    fn test_create_unknown_protocol_fails() {
        let protocol = create_protocol("unknown", None);
        assert!(protocol.is_err());
    }
}
